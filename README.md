...
├── modexp_gcw.go             # GCW multi-exponentiation (double and fourfold)
...
├── modexp_gcw_test.go        # GCW multi-exponentiation tests + benchmarks
├── bench_2048_test.go        # 2048-bit benchmarks
├── bench_3072_test.go        # 3072-bit benchmarks
...
# GCW multi-exponentiation tests (double and fourfold)
AVX512_TEST=1 go test -v -run "TestModExpDouble|TestModExpFourfold"

# Correctness — all strategies, all bit sizes (64 to 2048-bit)
AVX512_TEST=1 go test -v -run "TestModExp.*Random"

# 3072-bit correctness
AVX512_TEST=1 go test -v -run "3072"
...
# Benchmarks — 3072-bit
AVX512_TEST=1 go test -bench "3072" -benchmem -count=3

# Comparison vs Notus (run from comparison/ directory)
cd comparison/
AVX512_TEST=1 go test -v -run TestCorrectness
AVX512_TEST=1 go test -bench BenchmarkComparison -benchmem -benchtime 5s -count=3
```
...
## Comparison with jiajunxin/multiexp

A fair benchmark was run against [jiajunxin/multiexp](https://github.com/jiajunxin/multiexp), a scalar Montgomery implementation that uses thread-level parallelism and GCW (Greatest Common Words) to avoid redundant multiplications across multiple exponents. Both libraries use the same precomputed table strategy, so the comparison isolates the effect of the multiplication backend — AVX512+RNS (VROOM) vs scalar Montgomery (jiajunxin).

### Go 1.22+ compatibility

jiajunxin/multiexp uses internal `math/big` symbols via assembly trampolines (`shrVU`, `divWW`, `mulWW`, `reciprocalWord`, `nlz`) that were removed in Go 1.22+. To fix this, the library was forked at `github.com/dimitrisbaltzis/multiexp`, the assembly files (`arith_*.s`, `arith_decl.go`) were removed, and replaced with pure Go implementations. The `go.mod` in the `comparison/` directory uses a `replace` directive to point to the fork.

### Results (Intel Xeon Gold 6326 @ 2.90 GHz, single core vs single core)
...
VROOM wins in every single-thread scenario. The advantage grows with key size — from 2.7× at 1024-bit to 5.8× at 2048-bit for single exponentiation — consistent with AVX512+RNS parallelism becoming more valuable as the number of residues increases (22 at 1024-bit, 44 at 2048-bit).

The only scenario where jiajunxin wins is fourfold at 1024-bit with 64 threads (416 μs vs 791 μs), combining GCW and thread-level parallelism. At 2048-bit, VROOM single-thread (3,126 μs) is within 26% of jiajunxin 64-thread (2,476 μs).

Full analysis in [`comparison/benchmark_analysis.md`](comparison/benchmark_analysis.md).
